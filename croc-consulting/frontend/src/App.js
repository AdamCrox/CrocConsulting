...
// Quote Submit Portal
function QuoteSubmitPortal({ onClose, clientId, onSubmit }) {
  const [tab, setTab] = useState("upload");
  const [files, setFiles] = useState([]);
  const [contact, setContact] = useState({ name: "", email: "", company: "", phone: "", project: "", timeframe: "", quote_type: "Binding", urgency: "Standard", notes: "", site_location: "", standards: "" });
  ...
  const doSubmit = async () => {
    ...
    try {
      ...
      for (const f of files) {
        ...
        await axios.post(`${API_URL}/submissions/upload`, formData);
      }
      
      if (onSubmit) onSubmit({ id: submissionId, ...contact });
      setSubmitted(true);
    } catch (e) {
      console.error(e);
      ...
      {submitted ? (
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 32px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                    <div>
                      ...
                      <select value={contact.quote_type} onChange={e => setContact({ ...contact, quote_type: e.target.value })} style={{ ...inp, appearance: "auto" }} data-testid="select-quote-type">
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      <div>
                        <label style={{ fontSize: 13, fontWeight: 500, color: D, display: "block", marginBottom: 6 }}>Quote type</label>
                        <select value={contact.quote_type} onChange={e => setContact({ ...contact, quote_type: e.target.value })} style={{ ...inp, appearance: "auto" }} data-testid="fillout-quote-type">
                          <option>Binding</option>
                          <option>Non-binding</option>
                        </select>
                      </div>
                    ...
                    <div>
                      ...
                      <input type="text" value={contact.site_location} onChange={e => setContact({ ...contact, site_location: e.target.value })} style={inp} onFocus={onF} onBlur={onB} data-testid="fillout-site-location" />
                    </div>
                    <div>
          ...
          <div style={{ borderTop: "1px solid rgba(0,0,0,0.08)", marginTop: 40, paddingTop: 24, display: "flex", justifyContent: "flex-end", gap: 12 }}>
            ...
            <button onClick={doSubmit} disabled={uploading} style={{ padding: "12px 24px", border: "none", borderRadius: 12, background: B, fontSize: 15, fontWeight: 500, color: "#fff", cursor: uploading ? "wait" : "pointer" }} data-testid="submit-quote-btn">{uploading ? "Submitting..." : "Submit quote request"}</button>
          </div>
        </div>
      )}
    </div>
...
function QuotePortal({ onClose }) {
  ...
  const doLogin = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/auth/login`, { client_id: loginId, password: loginPw });
      if (res.data.success) {
        setClientId(res.data.client_id);
        setClientName(res.data.company);
        setLoggedIn(true);
        setLoginError("");
        // Store in session
        sessionStorage.setItem("clientId", res.data.client_id);
        sessionStorage.setItem("clientName", res.data.company);
        // Load quotes and submissions
        const [quotesRes, subsRes] = await Promise.all([
          axios.get(`${API_URL}/quotes/${loginId}`),
          axios.get(`${API_URL}/submissions/${loginId}`)
        ]);
  ...
  const approveQuote = async (quoteId) => {
    try {
      await axios.post(`${API_URL}/quotes/${quoteId}/approve`);
      setQuotes(q => q.map(quote => quote.id === quoteId ? { ...quote, approved: true, current_step: 6 } : quote));
      if (selectedQuote?.id === quoteId) {
        setSelectedQuote({ ...selectedQuote, approved: true, current_step: 6 });
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const submitRFI = async (quoteId, rfiIndex) => {
    ...
    if (!response) return;
    try {
      await axios.post(`${API_URL}/quotes/${quoteId}/rfi/${rfiIndex}/respond`, { response });
      const today = new Date().toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" });
      setQuotes(q => q.map(quote => {
        if (quote.id === quoteId) {
          const newRfi = [...quote.rfi];
          newRfi[rfiIndex] = { ...newRfi[rfiIndex], response, response_date: today };
  ...
  const generateAI = async (submissionId) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/submissions/${submissionId}/generate-ai`);
      setSubmissions(s => s.map(sub => sub.id === submissionId ? { ...sub, ai_output: res.data.output } : sub));
    } catch (e) {
      console.error(e);
      alert("Failed to generate AI quote. Please try again.");
    }
    setGeneratingAI(g => ({ ...g, [submissionId]: false }));
  };

  const updateAIOutput = async (submissionId, newOutput) => {
    setSubmissions(s => s.map(sub => sub.id === submissionId ? { ...sub, ai_output: newOutput } : sub));
    try {
      await axios.put(`${API_URL}/submissions/${submissionId}/ai-output`, { ai_output: newOutput });
    } catch (e) {
      console.error(e);
    }
  };
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 32px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
            <div>
              ...
              <p style={{ fontSize: 14, color: G }}>{clientName} - Raised {selectedQuote.date_raised}</p>
            </div>
          ...
          {/* Warning banners */}
          {selectedQuote.type === "Binding" && selectedQuote.expiry_date && !selectedQuote.approved && (
            <div style={{ display: "flex", alignItems: "center", gap: 12, background: "#FEF3C7", border: "1px solid #FDE68A", borderRadius: 12, padding: "12px 16px", marginBottom: 24 }}>
              ...
              <span style={{ fontSize: 14, color: "#92400E" }}>PO must be received by {selectedQuote.expiry_date}</span>
            </div>
          )}
          ...
          {detailTab === "progress" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
              <ProgressTracker currentStep={selectedQuote.current_step} />
              <div style={{ background: LB, borderRadius: 16, padding: 28 }}>
                {[
                  ["Quote ID", selectedQuote.id],
                  ["Project", selectedQuote.project],
                  ["Client", clientName],
                  ["Date raised", selectedQuote.date_raised],
                  ["Quote type", selectedQuote.type],
                  ["Total value", fmt(total(selectedQuote.items))],
                  ["Status", selectedQuote.approved ? "Approved" : STEPS[selectedQuote.current_step]]
                ].map(([l, v], i, arr) => (
                ...
                {selectedQuote.current_step >= 4 && !selectedQuote.approved && (
              ...
              {selectedQuote.rfi.length === 0 ? (
                ...
                <div key={i} style={{ background: LB, borderRadius: 16, padding: 24, border: !rfi.response ? `1px solid ${AM}33` : "1px solid transparent" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: B }}>{rfi.from}</span>
                  ...
                  {rfi.response ? (
                    <div style={{ borderLeft: `3px solid ${GN}`, paddingLeft: 16, marginTop: 16 }}>
                      ...
                      <span style={{ fontSize: 12, color: G }}>{rfi.response_date}</span>
                    </div>
                  ) : (
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "32px 32px" }}>
          ...
          {portalTab === "active" ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {quotes.length === 0 ? (
                <p style={{ fontSize: 15, color: G, textAlign: "center", padding: 40 }}>No active quotes</p>
              ) : quotes.map(q => (
                <div key={q.id} onClick={() => { setSelectedQuote(q); setDetailTab("progress"); }} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "24px 28px", background: "#fff", border: "1px solid rgba(0,0,0,0.06)", borderRadius: 16, cursor: "pointer", transition: "all 0.2s" }}
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 14, color: G }}>{clientName} - Raised {q.date_raised}</p>
                  </div>
                  <div style={{ textAlign: "right" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 600, padding: "4px 10px", borderRadius: 980, background: sub.contact.quote_type === "Binding" ? BL : "#FEF3C7", color: sub.contact.quote_type === "Binding" ? B : "#92400E" }}>{sub.contact.quote_type}</span>
                      </div>
                      <h3 style={{ fontSize: 17, fontWeight: 600, color: D, marginBottom: 4 }}>{sub.contact.project || "Untitled project"}</h3>
                      <p style={{ fontSize: 14, color: G }}>{sub.contact.company} - {new Date(sub.created_at).toLocaleDateString("en-AU", { day: "numeric", month: "long", year: "numeric" })}</p>
                      <p style={{ fontSize: 13, color: G, marginTop: 4 }}>{sub.files?.length > 0 ? `${sub.files.length} file(s) uploaded` : `${sub.items?.length || 0} line item(s)`}</p>
                    </div>
                    ...
                    <div style={{ marginTop: 16 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          ...
                          <button onClick={() => copyToClipboard(sub.ai_output)} style={{ padding: "6px 12px", border: "none", borderRadius: 8, background: LB, fontSize: 12, fontWeight: 500, color: G, cursor: "pointer" }} data-testid={`copy-${sub.id}`}>Copy</button>
                        </div>
                      </div>
                      <textarea value={sub.ai_output} onChange={e => updateAIOutput(sub.id, e.target.value)} style={{ width: "100%", minHeight: 300, padding: 16, borderRadius: 12, border: "1px solid rgba(0,0,0,0.08)", fontSize: 13, fontFamily: "monospace", lineHeight: 1.6, resize: "vertical", outline: "none", boxSizing: "border-box" }} data-testid={`ai-output-${sub.id}`} />
                    </div>
                  )}
                </div>
              ))}
...
function App() {
  ...
  if (showSubmit) return <QuoteSubmitPortal onClose={() => setShowSubmit(false)} clientId={sessionStorage.getItem("clientId")} />;
    ...
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', Helvetica, sans-serif", WebkitFontSmoothing: "antialiased", margin: 0, padding: 0 }}>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => { setShowForm(false); setSent(false); }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 48, maxWidth: 480, width: "90%", boxShadow: "0 24px 80px rgba(0,0,0,0.18)", animation: "slideUp 0.3s ease", position: "relative" }} onClick={e => e.stopPropagation()}>
